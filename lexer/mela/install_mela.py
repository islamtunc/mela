...
install_dir = os.path.join(os.environ.get('ProgramFiles', 'C:/Program Files'), 'MelaLang')

print(f"Installing Mela to: {install_dir}\n")

# Create install directory if it doesn't exist
os.makedirs(install_dir, exist_ok=True)

# List of files/folders to copy (relative to src_dir)
items = [
    'lexer/lexer.asm',
    'lexer/hello_mela.py',
...
def copy_item(item):
    ...
    dst_path = os.path.join(install_dir, os.path.basename(item))
...
def try_run_script(script_path, description):
...
lexer_build_script = os.path.join(src_dir, 'lexer', 'build_lexer_win.cmd')
if os.path.exists(lexer_build_script):
    if not try_run_script(lexer_build_script, 'Build lexer.exe (Windows)'):
        # If failed, try build_and_run_lexer.cmd (WSL/Linux)
        lexer_build_script2 = os.path.join(src_dir, 'lexer', 'build_and_run_lexer.cmd')
        if os.path.exists(lexer_build_script2):
            try_run_script(lexer_build_script2, 'Build lexer (WSL/Linux)')
        else:
            print('No build script found for lexer.')
else:
    print('No build_lexer_win.cmd found, skipping lexer.exe build.')
...
try:
    ...
    with winreg.OpenKey(winreg.HKEY_CURRENT_USER,
        ...
        if install_dir not in current_path:
            ...
            print("Added MelaLang to user PATH. Please restart your terminal or computer.")
        else:
            print("MelaLang is already in your PATH.")
except Exception as e:
...
class InstallerGUI:
    ...
    def run_install(self):
        try:
            ...
            self.log(f"Installing Mela to: {install_dir}\n")
            os.makedirs(install_dir, exist_ok=True)
            # Try to build lexer.exe using build_lexer_win.cmd first
            lexer_build_script = os.path.join(src_dir, 'lexer', 'build_lexer_win.cmd')
            if os.path.exists(lexer_build_script):
